...
class AdaptiveResGroup(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, n_blocks):
        ...
        module_group = [
            AdaptiveResBlock(in_channels, out_channels, kernel_size=kernel_size)
...
class SolverHR(nn.Module):
    def __init__(self, scale, n_channels=64, n_blocks=5, n_groups=3):
        ...
        module_body = [
            AdaptiveResGroup(n_channels, n_channels, kernel_size=3, n_blocks=n_blocks)
            for _ in range(n_groups)
        ]
        module_tail = [
            nn.Conv2d(n_channels, n_channels, kernel_size=3, padding=3 // 2),
            nn.Conv2d(n_channels, 3 * scale * scale, kernel_size=3, padding=3 // 2),
            nn.PixelShuffle(scale),
            nn.Conv2d(3, 3, kernel_size=5, padding=5 // 2),
        ]
        module_skip = [
            nn.Conv2d(3, 3 * scale * scale, kernel_size=5, padding=5 // 2),
            nn.PixelShuffle(scale),
            nn.Conv2d(3, 3, kernel_size=5, padding=5 // 2),
        ]
...
class Downsampler(nn.Module):
    def __init__(self, scale):
        ...
        if scale == 2:
            module_downsampler = [
                nn.Conv2d(3, 32, kernel_size=3, padding=3 // 2),
                nn.Conv2d(
                    32,
                    32,
                    kernel_size=3,
...
class SolverLR(nn.Module):
    def __init__(self):
        ...
        module_conv = [
            nn.Conv2d(3 * 2, 64, kernel_size=3, padding=3 // 2),
...
class SolverEM(nn.Module):
    def __init__(self, n_inputs):
        ...
        module_em = [
            nn.Conv2d(3 * n_inputs, 64, kernel_size=3, padding=3 // 2),
            nn.ReLU(True),
            nn.Conv2d(64, 3, kernel_size=3, padding=3 // 2),
        ]
