...
class OverNet(nn.Module):
    def __init__(self, channels, rgb_mean=(0.4488, 0.4371, 0.4040)):
        ...
        self.longRangeSkip = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), wn(nn.Conv2d(channels, channels, 1)), nn.ReLU(True)
        )
        self.add = AdaptiveAdd()
    ...
    def forward(self, img, out_size):
        ...
        h = self.OSMBlock(h)
        f_x4_1 = F.interpolate(h, out_size, mode="bicubic", align_corners=False)
        f_x4_2 = F.interpolate(img, out_size, mode="bicubic", align_corners=False)
